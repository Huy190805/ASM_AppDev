...
public class MainActivity extends AppCompatActivity {
    ...
    private void sendBudgetNotification(String category, String message) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU &&
                ContextCompat.checkSelfPermission(this, android.Manifest.permission.POST_NOTIFICATIONS)
                        != PackageManager.PERMISSION_GRANTED) {
            return; // Don't show if not permitted
        }

        NotificationCompat.Builder builder = new NotificationCompat.Builder(this, "expense_alerts")
                .setSmallIcon(android.R.drawable.ic_dialog_alert)
                .setContentTitle("Budget Alert: " + capitalize(category))
                .setContentText(message)
                .setPriority(NotificationCompat.PRIORITY_HIGH)
                .setAutoCancel(true);

        NotificationManagerCompat.from(this).notify(category.hashCode(), builder.build());
    }


    private void checkCategoryNotifications() {
        DatabaseReference userRef = FirebaseDatabase.getInstance().getReference("users").child(currentUsername);

        userRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                int totalBudget = snapshot.child("MonthlyBudget").child("amount").getValue(Integer.class) != null
                        ? snapshot.child("MonthlyBudget").child("amount").getValue(Integer.class)
                        : 0;
                ...
                if (snapshot.child("categories").exists()) {
                    for (DataSnapshot catSnap : snapshot.child("categories").getChildren()) {
                        String name = catSnap.getKey();
                        Integer limit = catSnap.child("amount").getValue(Integer.class);
                        if (name != null && limit != null) {
                            categoryBudget.put(name, limit);
                        }
                    }
                }
                ...
                for (String cat : categorySpent.keySet()) {
                    int spent = categorySpent.get(cat);
                    int limit = categoryBudget.getOrDefault(cat, 0);
                    if (limit == 0) continue;

                    float percent = (spent * 100f) / limit;

                    if (percent >= 100) {
                        sendBudgetNotification(cat, "❗ You've exceeded your " + capitalize(cat) + " budget");
                    } else if (percent >= 80) {
                        sendBudgetNotification(cat, "⚠️ You’re nearing your " + capitalize(cat) + " budget (" + (int) percent + "% used)");
                    }
                }
            }
